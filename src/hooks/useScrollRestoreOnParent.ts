...
export const useScrollRestoreOnParent = (
  ...
  useEffect(() => {
    ...
    const save = () => {
      ...
      const currentScroll = String(scrollEl.scrollTop);

      if (id !== undefined && id !== null) {
        let map: Record<string | number, string> = {};
        try {
          map = JSON.parse(rawValue || '{}');
          if (typeof map !== 'object') map = {}; 
        } catch {
          map = {};
        }

        map[id] = currentScroll;
        sessionStorage.setItem(storageKey, JSON.stringify(map));
      } else {
        sessionStorage.setItem(storageKey, currentScroll);
      }
    };
  ...
  }, [anchorRef, enabled, key, saveThrottleFrame, saveThrottleMs, saveOnPageHide, id, storageKey]);
  ...
  useLayoutEffect(() => {
    ...
    const apply = () => {
      const maxTop = scrollEl.scrollHeight - scrollEl.clientHeight;
      if (maxTop < top) return; // 아직 충분히 안 자랐으면 복원 안 함
